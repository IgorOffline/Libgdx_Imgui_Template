...
public class MaxDXDY implements CalculateHeuristic {

    @Override
    public int calculate(Position source, Position destination) {

        var heuristicEstimate = 2;
        return heuristicEstimate * (Math.max(Math.abs(source.row() - destination.row()), Math.abs(source.column() - destination.column())));
    }
}
